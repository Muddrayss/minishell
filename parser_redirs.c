...
/*   Updated: 2024/01/14 14:08:12 by egualand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_parser *content_par, t_data *data)
{
	...
	if (token_streak == 2 && next_token == REDIR_L)
	{
		...
		add_left_right_filenames(&redir_content->input.filename,
	...
	{
		add_left_right_fds(&redir_content->output.fd, next_cmd_elem->str.cmd,
			RIGHT);
...
void	handle_redir_r(t_list *lexered_params, t_parser *content_par,
		t_data *data)
{
	t_redir				*redir_content;
	t_lexer				*next_cmd_elem;
	...
	if (add_left_right_fds(&redir_content->output.fd, next_cmd_elem->str.cmd,
			RIGHT) == -1)
		add_left_right_filenames(&redir_content->output.filename,
			next_cmd_elem->str.cmd, RIGHT, data);
